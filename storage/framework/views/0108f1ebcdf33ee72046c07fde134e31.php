<?php $__env->startSection('page_title', 'Ambil KRS'); ?>
...
<?php $__env->startSection('content'); ?>
    ...
    <?php if(!$data['is_semester_active']): ?>
        <div class="nb-alert nb-alert-warning mb-6 flex items-center gap-2">
    ...
    <div class="nb-bento mb-6" style="grid-template-columns: repeat(auto-fit, minmax(240px, 1fr));">
        <div class="nb-stat nb-stat--info nb-stat--ribbon">
            ...
            <div class="nb-stat-value" id="displaySemesterAktif">
                <?php if($data['is_semester_active']): ?>
                    <?php echo e($data['semester_label']); ?>
        ...
        <div class="nb-stat nb-stat--warning nb-stat--ribbon">
            <div class="flex items-center gap-3">
                <div class="nb-stat-icon">
                    <?php if($data['status_krs'] === 'Sudah Mengajukan'): ?>
    ...
    <div class="nb-card mb-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                ...
                <select id="filterTahun" <?php if(!$data['is_semester_active']): ?> disabled <?php endif; ?>>
                    <?php if($data['is_semester_active']): ?>
                        <option value="<?php echo e($data['tahun_ajaran_aktif']); ?>" selected>
                            <?php echo e($data['tahun_ajaran_aktif']); ?> (Aktif)
                        </option>
                    <?php else: ?>
            ...
            <div>
                ...
                <select id="filterSemester" <?php if(!$data['is_semester_active']): ?> disabled <?php endif; ?>>
                    <?php if($data['is_semester_active']): ?>
                        <option value="<?php echo e($data['semester_label']); ?>" selected>
                            <?php echo e($data['semester_label']); ?> (Aktif)
                        </option>
                    <?php else: ?>
                        <option value="">-- Tidak ada semester aktif --</option>
                    <?php endif; ?>
            ...
            <div class="flex items-end">
                <button
                    ...
                    <?php if(!$data['is_semester_active']): ?> disabled <?php endif; ?>
                >
    ...
    <form method="POST" action="<?php echo e(route('pages.mahasiswa.store-krs')); ?>" id="formKrs">
        <?php echo csrf_field(); ?>

        <input type="hidden" name="semester" id="inputSemester" value="<?php echo e($data['semester_label'] ?? ''); ?>">
        <input type="hidden" name="tahun_ajaran" id="inputTahun" value="<?php echo e($data['tahun_ajaran_aktif'] ?? ''); ?>">
            ...
            <div class="nb-section-header">
                <div>
                    <span class="nb-eyebrow" style="color:var(--color-accent-soft);">
                        Paket Semester <span id="labelSemesterWajib"><?php echo e($data['semester_label'] ?? '-'); ?></span>
                    </span>
...
<script>
    ...
    const semesterAktifData = {
        label: <?php echo json_encode($data['semester_label'] ?? '', 15, 512) ?>,
        ke: <?php echo json_encode($data['semester_aktif'] ?? '', 15, 512) ?>,
        tahun: <?php echo json_encode($data['tahun_ajaran_aktif'] ?? '', 15, 512) ?>,
        isActive: <?php echo json_encode($data['is_semester_active'] ?? false, 15, 512) ?>
    };
    ...
    function loadPaketSemester() {
        ...
        fetch(`<?php echo e(route('pages.mahasiswa.api.paket-semester')); ?>?semester=${encodeURIComponent(semester)}&tahun_ajaran=${encodeURIComponent(tahun)}`)
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layouts.mahasiswa', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\laravel\SistemPengolahanKRS-KHS\resources\views/pages/mahasiswa/ambil-krs.blade.php ENDPATH**/ ?>