...
@section('page_title', 'Edit Paket Mata Kuliah')
...
@section('content')
    ...
    <div class="nb-page-header">
        <div>
            ...
            <h1 class="mt-2">Edit Paket Mata Kuliah</h1>
            <p>Ubah paket mata kuliah {{ $paketMK->nama_paket ?? 'N/A' }}</p>
        </div>
    ...
    <div class="nb-card max-w-4xl">
        <form action="{{ route('pages.admin.paketmk.update', $paketMK->id) }}" method="POST">
            @csrf
            @method('PUT')

            <div class="nb-section-header mb-8">
                <h2>Informasi Paket Mata Kuliah</h2>
                <p class="text-muted">Update data paket mata kuliah di bawah ini</p>
            </div>

            {{-- Nama Paket --}}
            <div class="mb-6">
                <label class="nb-label">Nama Paket <span class="text-danger">*</span></label>
                <input type="text" name="nama_paket" value="{{ old('nama_paket', $paketMK->nama_paket ?? '') }}" placeholder="Paket Semester 1 Teknik Informatika" required
                ...
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <div class="text-sm text-info-dark mt-1">
                            <div>Total Mata Kuliah: <span id="jumlahMk" class="font-bold">{{ $paketMK->jumlah_mk ?? 0 }}</span></div>
                            <div>Total SKS: <span id="totalSks" class="font-bold">{{ $paketMK->total_sks ?? 0 }}</span></div>
                        </div>
                    </div>
                </div>
            </div>
