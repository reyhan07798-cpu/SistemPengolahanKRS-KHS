...
@section('content')
    ...
    <div class="nb-bento" style="grid-template-columns: repeat(auto-fit, minmax(240px, 1fr));">
        <div class="nb-stat nb-stat--info nb-stat--ribbon">
            <div class="flex items-center gap-3">
                <div class="nb-stat-icon"><span class="material-symbols-outlined filled">event</span></div>
                <p class="nb-stat-label">Semester Aktif</p>
            </div>
            <div class="nb-stat-value" id="displaySemesterAktif">Ganjil</div>
        </div>
        <div class="nb-stat nb-stat--accent nb-stat--ribbon">
            <div class="flex items-center gap-3">
                <div class="nb-stat-icon"><span class="material-symbols-outlined filled">verified</span></div>
        ...
        <div class="nb-stat nb-stat--warning nb-stat--ribbon">
            <div class="flex items-center gap-3">
                <div class="nb-stat-icon"><span class="material-symbols-outlined filled">pending</span></div>
    ...
    <form method="POST" action="{{ route('pages.mahasiswa.store-krs') }}" id="formKrs">
        @csrf
        <input type="hidden" name="semester" id="inputSemester" value="Ganjil">
        ...
        <div id="containerWajib" class="nb-card-flat mb-6 hidden">
            <div class="nb-section-header">
                <div>
                    <span class="nb-eyebrow" style="color:var(--color-accent-soft);">Paket Semester <span id="labelSemesterWajib">Ganjil</span></span>
    ...
    <script>
        ...
        // Data dummy berdasarkan Ganjil/Genap
        const dummyData = {
            'Ganjil': {
                wajib: [
                    ...
                    { id: 5, kode: 'IF105', matkul: 'Pengantar Teknologi Informasi', dosen: 'Dr. Hendra Saputra, M.Kom', sks: 3, prasyarat: '-' },
                ],
                mengulang: []
            },
            'Genap': {
                wajib: [
                    { id: 6,  kode: 'IF201', matkul: 'Basis Data',                      dosen: 'Dr. Budi Santoso, M.T',    sks: 4, prasyarat: 'IF101' },
                    { id: 7,  kode: 'IF202', matkul: 'Pemrograman Web',                  dosen: 'Dr. Andi Wijaya, M.Kom',   sks: 4, prasyarat: 'IF101' },
                    { id: 8,  kode: 'IF203', matkul: 'Jaringan Komputer',                dosen: 'Dr. Citra Lestari, M.T',   sks: 4, prasyarat: 'IF102' },
                    { id: 9,  kode: 'IF204', matkul: 'Proyek Pembuatan Prototipe',       dosen: 'Dr. Dewi Kusuma, M.Kom',   sks: 4, prasyarat: '-' },
                    { id: 10, kode: 'IF205', matkul: 'Sistem Operasi',                   dosen: 'Dr. Eko Prasetyo, M.Kom',  sks: 3, prasyarat: 'IF101' },
                    { id: 11, kode: 'IF206', matkul: 'Struktur Data',                    dosen: 'Dr. Fitri Handayani, M.T', sks: 3, prasyarat: 'IF101' },
                ],
                mengulang: [
                ...
                row.innerHTML = `
                    <td class="text-center">
                        <input type="checkbox" name="mata_kuliah_ids[]" value="${mk.id}"
                            data-sks="${mk.sks}" class="chk-mk w-5 h-5 cursor-pointer"
        ...
        function hitungSks() {
            ...
            if (selectedSks > MAX_SKS) {
                ...
                warningInner.classList.replace('nb-alert-warning', 'nb-alert-danger');
                btnSubmit.disabled = true;
            } else if (selectedSks === MAX_SKS) {
                warning.classList.remove('hidden');
                warningText.textContent = `Anda telah mencapai batas maksimal ${MAX_SKS} SKS.`;
                warningInner.classList.replace('nb-alert-danger', 'nb-alert-warning');
