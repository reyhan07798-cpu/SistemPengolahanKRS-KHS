...
@if(session('success'))
    <div class="nb-alert nb-alert-success mb-6">{{ session('success') }}</div>
@endif
...
<div class="nb-card mb-6">
    ...
    <form method="GET" action="{{ route('dosen.mk.input-nilai') }}" id="filterForm">
                ...
                <select name="semester_id" id="selectSemester" onchange="this.form.submit()">
                    ...
                    @foreach($allSem as $sem)
                        <option value="{{ $sem->id }}" {{ $filterSemesterId == $sem->id ? 'selected' : '' }}>
                            {{ $sem->semester }} {{ $sem->tahun_ajaran }} @if($sem->is_active) ★ Aktif @endif
                        </option>
                    @endforeach
                </select>
            </div>
        ...
        <div class="mt-3">
            ...
            @else
                <span class="nb-badge nb-badge-stable py-2 px-4">🔒 Semester Tidak Aktif — Read Only</span>
            @endif
        </div>
        @endif
    </form>
...
<div class="nb-card mb-6" id="bobotCard">
    ...
    <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        @php $val = $mahasiswaList[0]['bobot_'.$b['key']] ?? ['tugas'=>20,'praktikum'=>15,'uts'=>30,'uas'=>30,'kehadiran'=>5][$b['key']]; @endphp
        <div>
            <label class="nb-label">{{ $b['label'] }}</label>
            <input type="number" id="bobot_{{ $b['key'] }}" value="{{ $val }}"
                   min="0" max="100" step="1" class="text-center bobot-input w-full"
        ...
        <table class="nb-table" id="tabelNilai">
            <thead>
                <tr>
                    ...
                    <th>NIM / Nama</th>
                    <th class="text-center" style="min-width:80px">Tugas<br><small id="hdr_tugas" class="text-muted font-normal">({{ $mahasiswaList[0]['bobot_tugas'] ?? 20 }}%)</small></th>
                    <th class="text-center" style="min-width:80px">Praktikum<br><small id="hdr_praktikum" class="text-muted font-normal">({{ $mahasiswaList[0]['bobot_praktikum'] ?? 15 }}%)</small></th>
                    <th class="text-center" style="min-width:80px">UTS<br><small id="hdr_uts" class="text-muted font-normal">({{ $mahasiswaList[0]['bobot_uts'] ?? 30 }}%)</small></th>
                    <th class="text-center" style="min-width:80px">UAS<br><small id="hdr_uas" class="text-muted font-normal">({{ $mahasiswaList[0]['bobot_uas'] ?? 30 }}%)</small></th>
                    <th class="text-center" style="min-width:80px">Kehadiran<br><small id="hdr_kehadiran" class="text-muted font-normal">({{ $mahasiswaList[0]['bobot_kehadiran'] ?? 5 }}%)</small></th>
                    <th class="text-center">Nilai Akhir</th>
                    <th class="text-center">Grade</th>
                    @if(!$isReadOnly)<th class="text-center">Aksi</th>@endif
                </tr>
            </thead>
            <tbody>
            ...
            <tr id="row_{{ $mhs['id'] }}">
                ...
                <td class="text-center p-1">
                    @if(!$isReadOnly)
                    <input type="number" id="{{ $k }}_{{ $mhs['id'] }}"
                           value="{{ $mhs['nilai_'.$k] ?? '' }}"
                    ...
                    @else
                    <span class="text-sm font-medium">{{ $mhs['nilai_'.$k] ?? '—' }}</span>
                    @endif
                </td>
                @endforeach
                ...
                @if(!$isReadOnly)
                <td class="text-center">
                    <button onclick="simpanSatu({{ $mhs['id'] }})" id="btn_{{ $mhs['id'] }}"
                            class="nb-btn nb-btn-primary nb-btn-sm">
                        <span class="material-symbols-outlined" style="font-size:14px;">save</span>
                    </button>
                    <span id="ok_{{ $mhs['id'] }}" class="text-xs text-green-600 hidden ml-1">✓</span>
                </td>
                @endif
            </tr>
            @endforeach
...
<div class="nb-card text-center py-12">
    ...
    <p class="text-muted mt-2">Belum ada mahasiswa yang KRS-nya disetujui untuk mata kuliah ini{{ $filterKelas ? ' di kelas '.$filterKelas : '' }}.</p>
</div>
@else
<div class="nb-card text-center py-12">
...
function onMKChange(sel) {
    ...
    if (!kodeMk) { document.getElementById('filterForm').submit(); return; }
    
    // Fetch kelas berdasarkan kode_mk
    fetch(URL_KELAS + '?kode_mk=' + kodeMk)
        .then(r => r.json())
        .then(data => {
...
function hitungBaris(id) {
    ...
    const akhir = (v('tugas')*b.tugas + v('praktikum')*b.praktikum + v('uts')*b.uts + v('uas')*b.uas + v('kehadiran')*b.kehadiran) / 100;
...
async function simpanSatu(id) {
    ...
    fd.append('kode_mk', KODE_MK); // Kirim kode_mk, bukan id
    fd.append('kelas', KELAS);     // Kirim kelas juga
    ...
    try {
        ...
        // Jika server bukan return 200 OK
        if (!res.ok) {
            const errorText = await res.text();
            console.error("Error dari Server:", errorText);
            alert('Gagal menyimpan! Server merespons dengan error. Buka Console Browser (F12) untuk detail.');
            return;
        }
