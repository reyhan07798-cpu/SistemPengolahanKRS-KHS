@props(['dosen', 'routeUpdate', 'routePassword'])
...
<div class="nb-card mb-8">
    <div class="flex items-center gap-6 flex-wrap">
        ...
        <div class="min-w-0 flex-1">
            <span class="nb-eyebrow">{{ $dosen['role'] ?? 'Dosen' }}</span>
            <h1 class="nb-h1 mt-1" style="font-size: 2rem;">{{ $dosen['nama'] }}</h1>
            <div class="flex items-center gap-2 mt-3 flex-wrap">
                <span class="nb-badge nb-badge-primary">
                    <span class="material-symbols-outlined" style="font-size:14px; margin-right:4px;">{{ $dosen['icon'] ?? 'badge' }}</span>
                    {{ $dosen['id_field'] ?? $dosen['nidn'] ?? $dosen['nip'] }}
                </span>
...
<div class="nb-card mb-8">
    ...
    <form action="{{ $routeUpdate }}" method="POST" id="formProfil">
        @csrf
        @method('PUT')
...
<div class="nb-card">
    ...
    <form action="{{ $routePassword }}" method="POST" id="formPassword" class="hidden">
        @csrf
        @method('PUT')
