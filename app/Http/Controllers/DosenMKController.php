...
class DosenMKController extends Controller
{
    ...
    // Berdasarkan kolom dosen_nik di tabel mata_kuliahs
    ...
    public function index()
    {
        ...
        $mataKuliah = array_map(function ($mk) {
            $jumlahMahasiswa = DB::table('krs_mahasiswa')
                ->where('mata_kuliah_id', $mk->id)
                ->where('status', 'disetujui')
                ->count();
    ...
    public function inputNilai(Request $request)
    {
        ...
        $tahunAjaranList = DB::table('mata_kuliahs')
            ->where('dosen_nik', $nik)
        ...
        $mataKuliahList = DB::table('mata_kuliahs')
            ->where('dosen_nik', $nik)
            ->get([
                'id',
                'kode_mk',
        ...
        $kelasList = collect($mataKuliahList)
            ->pluck('kelas')
            ->unique()
            ->sort()
            ->values()
    ...
    public function simpanNilai(Request $request)
    {
        ...
        $mk = DB::table('mata_kuliahs')->find($mkId);
    ...
    public function lihatNilai(Request $request)
    {
        ...
        $daftarMK = DB::table('mata_kuliahs')
            ->where('dosen_nik', $nik)
            ->get([
                'id',
                'kode_mk',
        ...
        $kelasList = DB::table('mata_kuliahs')
            ->where('dosen_nik', $nik)
            ->whereNotNull('kelas')
            ->distinct()
            ->pluck('kelas')
        ...
        $query = DB::table('nilai')
            ->join('users', 'nilai.mahasiswa_id', '=', 'users.id')
            ->join('mata_kuliahs', 'nilai.mata_kuliah_id', '=', 'mata_kuliahs.id')
            ->where('nilai.dosen_nik', $nik)
            ->select(
                'users.id as mahasiswa_id',
                'users.name as nama',
                'mata_kuliahs.kode_mk',
                'mata_kuliahs.nama as nama_mk',
                'nilai.kelas',
        ...
        if ($filterMK) {
            $query->where('nilai.mata_kuliah_id', $filterMK);
        }

        if ($filterKelas) {
            $query->where('nilai.kelas', $filterKelas);
        }

        if ($filterTahunAjaran) {
            $query->where('nilai.tahun_ajaran', $filterTahunAjaran);
        }
        ...
        $stats = [
            ...
            'nilai_terinput'  => collect($mahasiswa)
                ->filter(fn ($m) => !is_null($m['nilai_akhir']))
                ->count(),
